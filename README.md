...
- **Windows Credential Manager** – Create, update, retrieve, list, and remove generic credentials on Windows
...
| Windows Credential Manager | ✅ Full Support | ✅ Full Support on Windows |
...
$credential = Get-SecureStoreCredential -TargetName 'SecureStore:Api' -AsCredential
...
1. Publish from a clean staging folder that contains the files listed in `SecureStore.psd1`.
2. Validate before publishing:
   ```powershell
   Test-ModuleManifest -Path .\SecureStore.psd1
   Invoke-ScriptAnalyzer -Path . -Recurse -Settings PSGallery
   Invoke-Pester -Script .\tests\SecureStore.Tests.ps1
   Publish-Module -Path . -Repository PSGallery -NuGetApiKey '<key>' -WhatIf
   ```
3. Remove `-WhatIf` only after manifest, analyzer, and tests pass on Windows PowerShell 5.1 and PowerShell 7+.
...
These commands manage Windows Credential Manager generic credentials. They are available only on Windows and use the current user's credential vault.

```powershell
# Create a new credential. Use -Force only when overwriting is intended.
New-SecureStoreCredential -TargetName 'SecureStore:Api' -UserName 'api-user' -Password 'token' -Persistence LocalMachine

# Update or create a credential.
Set-SecureStoreCredential -TargetName 'SecureStore:Api' -UserName 'api-user' -Password 'new-token'

# Retrieve as metadata plus SecureString, PSCredential, or explicit plain text.
Get-SecureStoreCredential -TargetName 'SecureStore:Api'
Get-SecureStoreCredential -TargetName 'SecureStore:Api' -AsCredential
Get-SecureStoreCredential -TargetName 'SecureStore:Api' -AsPlainText

# List metadata only. Secrets are not returned by list operations.
...
| Command | Purpose |
|---------|---------|
| `New-SecureStoreCredential` | Creates a generic credential and blocks accidental overwrite unless `-Force` is used |
| `Set-SecureStoreCredential` | Creates or updates a generic credential |
| `Get-SecureStoreCredential` | Retrieves a credential as metadata with SecureString, `PSCredential`, or plain text |
| `Get-SecureStoreCredentialList` | Lists credential metadata without exposing secrets |
| `Remove-SecureStoreCredential` | Deletes a generic credential and supports `-WhatIf` / `-Confirm` |

...
function New-SecureStoreCertificate {
  [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSAvoidUsingPlainTextForPassword', '', Justification = 'The parameter accepts SecureString and string for compatibility with existing automation; values are converted to SecureString before use.')]
  ...
  process {
    ...
    try {
      ...
      if ($Algorithm -eq 'RSA') {
        ...
        $certificateParams['KeySpec'] = 'Signature'
      }
      else {
        if ($script:IsWindowsPlatform) {
          $certificateParams['Provider'] = 'Microsoft Software Key Storage Provider'
        }

        $selfSignedParameters = (Get-Command -Name New-SelfSignedCertificate).Parameters
        if ($selfSignedParameters.ContainsKey('CurveName')) {
          $certificateParams['KeyAlgorithm'] = 'ECDSA'
          $certificateParams['CurveName'] = $CurveName
          if ($selfSignedParameters.ContainsKey('CurveExportPolicy')) {
            $certificateParams['CurveExportPolicy'] = 'Exact'
          }
        }
        else {
          $certificateParams['KeyAlgorithm'] = "ECDSA_$CurveName"
          if ($selfSignedParameters.ContainsKey('CurveExport')) {
            $certificateParams['CurveExport'] = 'CurveName'
          }
        }
      }

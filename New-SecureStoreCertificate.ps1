...
function New-SecureStoreCertificate {
  ...
  process {
    ...
    try {
      ...
      $pemPath = Join-Path -Path (Split-Path -Path $pfxPath -Parent) -ChildPath (
        "{0}.pem" -f [System.IO.Path]::GetFileNameWithoutExtension($pfxPath)
      )
      ...
      $textExtensions = @()
      $sanComponents = @()
      if ($DnsName) { $sanComponents += ($DnsName | ForEach-Object { "dns=$_ " -replace ' $', '' }) }
      if ($IpAddress) { $sanComponents += ($IpAddress | ForEach-Object { "ipaddress=$_ " -replace ' $', '' }) }
      if ($Email) { $sanComponents += ($Email | ForEach-Object { "email=$_ " -replace ' $', '' }) }
      if ($Uri) { $sanComponents += ($Uri | ForEach-Object { "url=$_ " -replace ' $', '' }) }
